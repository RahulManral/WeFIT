...
const Home = () => {
  ...
  const variants = {
    hidden: { x: '8%', opacity: 0 }, // Start from 50% to the right
    visible: { x: 0, opacity: 1 },    // Move to the original position
  };

  return (
    <div className="md:h-custom-height-lg lg:h-custom-height-lg xl-custom-height-xl 2xl:h-custom-height-lg 4xl:h-custom-height-2xl flex items-center bg-gray-100 xs:m-4 lg:m-5 rounded-xl border-4 border-black">
      <main className="flex flex-col items-center justify-center w-full flex-1 xs:px-2 lg:px-64 text-center mb-24 mt-24">
        ...
        <motion.button
          className="border-4 py-4 px-10 text-lg font-bold rounded-2xl bg-black text-white mt-5 hover:bg-white hover:text-black border-black transition duration-300 ease-in-out"
          initial="hidden"
          animate="visible"
          variants={variants}
          transition={{ duration: 0.5, delay: 0.4 }}
