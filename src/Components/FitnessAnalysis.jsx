...
export default function FitnessAnalysis({ analysis, onReset }) {
  return (
    <motion.div
      className="space-y-4 border-4 border-black m-4 px-6 py-14 rounded-xl"
      initial={{ opacity: 0, y: 20 }} // Initial state for the animation
      animate={{ opacity: 1, y: 0 }}   // Final state for the animation
      transition={{ duration: 0.5 }}     // Animation duration
    >
      <h2 className="text-2xl font-extrabold text-center -mt-2 mb-7 font-custom">Your Fitness Analysis</h2>
      <div>
        <p className="font-medium">BMI: {analysis.bmi}</p>
        <p className="font-medium">Category: {analysis.category}</p>
      ...
      <button
        onClick={onReset}
        className="w-full  mx-auto flex justify-center  py-2 border-4 border-black rounded-lg transition duration-300 shadow-sm text-sm font-medium text-white bg-black hover:bg-white hover:text-black "
      >
        Calculate Again
      </button>
    </motion.div>
